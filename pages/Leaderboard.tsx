...
const Leaderboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      const topUsers = await dbOps.getTopUsers(30);
      setUsers(topUsers);
      setLoading(false);
    };
    fetchLeaderboard();
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 py-6 animate-in fade-in duration-700">
      ...
      {users.length > 0 ? (
            ...
            {users.length >= 2 && (
              <div className="flex flex-col items-center gap-3">
                 <div className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-full border-4 border-slate-400 p-1`}>
                  <img src={users[1].photoUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${users[1].uid}`} className="rounded-full w-full h-full object-cover" alt="" />
            ...
            {users.length >= 1 && (
              <div className="flex flex-col items-center gap-3 scale-110">
                ...
                <div className={`relative w-20 h-20 sm:w-24 sm:h-24 rounded-full border-4 border-yellow-500 p-1`}>
                  <img src={users[0].photoUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${users[0].uid}`} className="rounded-full w-full h-full object-cover" alt="" />
            ...
            {users.length >= 3 && (
              <div className="flex flex-col items-center gap-3">
                 <div className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-full border-4 border-orange-800 p-1`}>
                  <img src={users[2].photoUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${users[2].uid}`} className="rounded-full w-full h-full object-cover" alt="" />
        ...
        <div className="text-center py-20 bg-slate-900/40 rounded-[40px] border border-slate-800 border-dashed">
          <p className="text-slate-500">No users found yet. Be the first to start ranking up!</p>
        </div>
      )}
