  ...
  return (
    <div className="bg-slate-900/60 backdrop-blur-xl flex flex-col h-full rounded-[24px] border border-slate-800/60 overflow-hidden shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-hide">
        ...
        {messages.map((msg) => (
          <div key={msg.id} className="flex gap-2.5 animate-in fade-in slide-in-from-bottom-1 duration-300">
      ...
      <div className="p-3 border-t border-slate-800/40 bg-slate-950/40">
        {user ? (
          <form onSubmit={handleSend} className="flex gap-2 bg-slate-900 rounded-xl p-1 border border-slate-800 focus-within:border-blue-500/50 transition-all">
            <input
              type="text"
              value={input}
          ...
          <div className="text-center py-1">
            <p className="text-[8px] font-black uppercase text-slate-700 tracking-widest">Sign in to comment</p>
          </div>
        )}
      </div>
    </div>
